...
def fitData(fileName):
    ...
    estYVals = a*xVals + b
    k = 1/a
    r_square = rSquare(yVals, estYVals)
    pylab.plot(xVals, estYVals, label = 'Linear fit, k = '
               + str(round(k, 5))+"R^2 = %f"%r_square)
...
def fitData1(fileName):
    ...
    estYVals = a*xVals + b
    r_squared_linear = rSquare(yVals, estYVals)
    pylab.plot(xVals, estYVals, label = 'Linear fit, R^2 =  %f'%r_squared_linear)
    a,b,c,d = pylab.polyfit(xVals, yVals, 3)
    estYVals = a*(xVals**3) + b*xVals**2 + c*xVals + d
    r_squared_cubic = rSquare(yVals, estYVals)
    pylab.plot(xVals, estYVals, label = 'Cubic fit, R^2 = %f'%r_squared_cubic)
    pylab.legend(loc = 'best')
