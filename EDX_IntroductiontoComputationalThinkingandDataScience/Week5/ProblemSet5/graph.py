...
class WeightedEdge(Edge):
    def __init__(self, src, dest, weight1, weight2):
        ...
        #assert(weight1 >= weight2), "Total Distance should be greater than or \
        #    equal to Outdoor Distance."
        self.total_dist = weight1
        self.outdoor_dist = weight2
    def __str__(self):
        string = super(WeightedEdge, self).__str__()
        return string + " ({},{})".format(self.total_dist,\
                                          self.outdoor_dist)
...
class WeightedDigraph(Digraph):
    ...
    def childrenOf(self, node):
        edges_ = [i[0] for i in self.edges[node]]
        return edges_
    def __str__(self):
        res = ''
        for k in self.edges:
