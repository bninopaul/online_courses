...
class Cluster(object):
    ...
    def averageLinkageDist(self, other):
        ...
        # TO DO
        m = float(len(self.points))
        n = float(len(other.points))
        total_dist = 0.0
        for point1 in self.points:
            for point2 in other.points:
                dist = point1.distance(point2)
                total_dist+=dist
        average_dist = total_dist/m*n
        return average_dist
    def members(self):
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        # TODO
        assert c1 in self.members and c2 in self.members and \
            c1.pointType ==c2.pointType
        combined = Cluster(c1.points+c2.points, c1.pointType)
        self.add(combined)
    ...
    def findClosest(self, linkage):
        ...
        for j in range(len(self.members)-1):
            first = [self.members[j] for i in range(len(self.members)-(j+1))]
            second = self.members[j+1:]
            combinations+=zip(first, second)

        nearest_cluster = None
        dist = float("inf")
        for pair in combinations:
            if linkage(pair[0],pair[1])<dist:
                dist = linkage(pair[0],pair[1])
                nearest_cluster = pair
    ...
    def mergeOne(self, linkage):
        ...
        # TODO
        clusters_similar = self.findClosest(linkage)
        self.mergeClusters(clusters_similar[0], clusters_similar[1])
        return clusters_similar
