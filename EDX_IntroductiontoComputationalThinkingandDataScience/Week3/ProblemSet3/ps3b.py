...
class Patient(object):
    ...
    def update(self):
        ...
        for vir in tuple(self.viruses):
            try:
                self.viruses.append(vir.reproduce(population_density))
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for trial in range(numTrials):
        list_virus = [SimpleVirus(maxBirthProb, clearProb) for i in range(numViruses)]
        patient = Patient(list_virus, maxPop)
        list_ = []
        for time in range(300):
            list_.append(patient.update())
        lists.append(list_)
    #data = numpy.array(lists)
    sum_pop_across_time = []
    for j in range(300):
        store = 0
        for k in lists:
            store+=k[j]
        sum_pop_across_time.append(store)


    #mean_pop_across_time = numpy.mean(data, axis = 0)
    mean_pop_across_time = [i/float(numTrials) for i in sum_pop_across_time]
    pylab.figure()
    pylab.plot(range(300), mean_pop_across_time, label = "simulation without drug")
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    res_lists = []
    for trial in range(numTrials):
        list_virus = [ResistantVirus(maxBirthProb, clearProb, resistances, mutProb)
        ...
        for time in range(150):
            ...
            if len(resistances)!=0:
                res_list_.append(len([i for i in patient.viruses if \
                                  len(i.resistances)==sum(i.resistances.values())]))
            else:
                res_list_.append(0)
    ...
    for j in range(300):
        ...
        for k in range(numTrials):
            store+=lists[k][j]
            res_store+=res_lists[k][j]
        sum_pop_across_time.append(store)
        sum_res_across_time.append(res_store)


    mean_pop_across_time = [i/float(numTrials) for i in sum_pop_across_time]
    mean_res_across_time = [i/float(numTrials) for i in sum_res_across_time]
    pylab.figure()
    pylab.plot(range(300), mean_pop_across_time, label = "total population")
    pylab.plot(range(300), mean_res_across_time, label = "resistant population")
    pylab.title("ResistantVirus simulation")
    pylab.xlabel("timesteps")
    pylab.ylabel("mean population")
    pylab.legend(loc = 0)
    pylab.show()
