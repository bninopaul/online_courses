...
class NaiveBayes:
  ...
  # TODO TODO TODO TODO TODO
    ...
    self.klasses = {}
    self.klasses_vocab = {}
    self.klasses_numwords = collections.defaultdict(lambda: 0)
  ...
  def classify(self, words):
    ...
    for klass in self.klasses:
        prob[klass] = math.log(self.klasses[klass]/self.N)
    for word in words:
        for klass in self.klasses:
            prob[klass] += math.log(self.klasses_vocab[klass][word]+1)
            prob[klass] -= math.log(self.klasses_numwords[klass]+len(self.V))

    return filter(lambda x: prob[x]==max(prob.values()), prob)[0]
  def addExample(self, klass, words):
     ...
     * Returns nothing
    """
    self.N += 1 #count the documents
    if klass in self.klasses:
        self.klasses[klass] += 1
    else:
        self.klasses[klass] = 1
        self.klasses_vocab[klass] = collections.defaultdict(lambda: 0)
    indices  = [(i,word) for i,word in enumerate(words) if re.search("^not$|n't|never", word) or re.match("[\.\?,!]", word)]
    ...
    for word in words:
        self.klasses_vocab[klass][word] += 1
        self.V[word]+=1
        self.klasses_numwords[klass] += 1
